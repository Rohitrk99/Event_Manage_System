...
const getVendorsByCategory = async (req, res) => {
  try {
    ...
    if (vendors.length === 0) {
      return res
        .status(404)
        .json({ message: `No vendors found for category: ${categoryParam}` });
    }
  ...
  try {
    ...
    if (
      ...
      return res
        ...
        .json({
          message:
            "Incomplete order details. Please check your cart and billing info.",
        });
    }
    ...
    return res
      .status(500)
      .json({ error: "Failed to place order. Internal Server Error." });
  }
};
...
const addGuest = async (req, res) => {
  try {
    ...
    if (!userId || !name) {
      return res
        .status(400)
        .json({ message: "User ID and Guest Name are required!" });
    }
    ...
    return res
      .status(201)
      .json({ message: "Guest added successfully!", guest: savedGuest });
  } catch (error) {
...
const updateGuest = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Guest updated!", guest: updatedGuest });
  } catch (error) {
    return res.status(500).json({ error: "Internal Server Error" });
  }
};
